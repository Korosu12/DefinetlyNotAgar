...
0) Core rules (locked for this architecture)
...
1) System A — Solana Program (Escrow + Accounting)
Purpose
...
2) System B — Attestation Layer (Signed Events Contract Accepts)
Purpose
...
3) System C — Authoritative Game Server (Simulation + Event Producer)
Purpose
...
4) System D — Web Client (Wallet + Gameplay + Cashout UX)
Purpose
...
deposit → join → play → exit & cashout → withdraw
all from the browser.

5) System E — Indexer + Database (Leaderboards + Auditing)
Purpose
...
6) System F — Hardening & Operations (Anti-abuse + Key Mgmt + Monitoring)
Purpose
...
The MVP milestone (your first real checkpoint)
MVP Goal: “End-to-end money loop with a minimal game”
